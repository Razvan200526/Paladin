...
describe('Auth Integration Test', () => {
  ...
  test('POST /api/auth/signup/email creates a user', async () => {
    const res = await app
      .getAppInstance()
      .request('http://localhost:3000/api/auth/signup/email', {
        method: 'POST',
  ...
  test('Manually verify user email', async () => {
    const client = new Client({
      connectionString:
        'postgresql://postgres:postgres@localhost:5432/paladin_test',
    });
  ...
  test('POST /api/auth/signin/email logs in the user', async () => {
    const res = await app
      .getAppInstance()
      .request('http://localhost:3000/api/auth/signin/email', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          email: testEmail,
